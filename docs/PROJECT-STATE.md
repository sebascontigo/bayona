...
| Campo | Valor | Etiqueta |
|---|---|---|
| Última fase completada | **FASE 6 — WORLD BUILDING** (5 bloques + cierre transversal, `3a9f511..f61c0a8`); 9.x completadas: 9.1 APROBADA (8.8/10), 9.2-A diagnóstico aceptado, 9.2-B APROBADA CON OBSERVACIONES | VERIFIED |
| Fase actual | **FASE 9.2-C — EVIDENCE INTEGRITY & EXPERIMENT GOVERNANCE** (2026-09-02, sobre `cafc915` = origin/main). Cierra las 3 observaciones del veredicto del arquitecto sobre 9.2-B: (1) **provenance de artifacts** — los afterAll de los specs de medición (three-network-audit, webvitals-lab) sobrescribían `artifacts/fase7a` INCONDICIONALMENTE desde Fase 7A (12+ sobrescrituras; clasificado **B diseño ambiguo**, ni latest declarado ni pérdida irreversible: Git conserva cada versión). Hallazgo de segundo orden: los artifacts pre-9.2-B eran corridas contra **DEV server** (config global :4173, URLs /src/) pese a que la config f7a declara `vite preview:4174` — la evidencia 9.2-B es la primera preview real. Fix: corridas latest → `artifacts/latest/` (gitignored); promoción a namespace SOLO con `EVIDENCE_NAMESPACE=<dir>`; `fase7a` restaurado a la evidencia 7A ORIGINAL (438ba3b: fuga 7A-01 viva 18/18 rutas); namespace `artifacts/fase9/9.2-b/` (evidencia 624b06f + runtime transcrito + manifest con SHAs reconciliados 6652392/ce74bb8/624b06f/cafc915); guard freeze + manifest en fase7aSceneGovernance (**13 tests**, chaos drill ROJO 1 failed → retirada a mano → VERDE 13/13). (2) **precisión semántica** — "texturas gratis / 100% del coste / E5" → scoped a laboratorio (**E5-LAB**) en comentario CSS + comentario guard + AUDIT-LOG 017 (marca de corrección visible, historia no borrada). (3) **cobertura del guard 9.2-B** — matriz de 10 escenarios verificada EMPÍRICAMENTE contra los regex reales (cubre body::after + selectores grain + shorthand multiline; NO cubre JS/Framer/WAAPI/otros pseudo-elementos — limitación documentada, guard no inflado a policía global). 0 producción tocada; grain exactamente como 9.2-B decidió (2 texturas, 0 animación; consolidación de texturas = decisión separada NO ejecutada). Entregable: FASE9.2C-EVIDENCE-GOVERNANCE.md + AUDIT-LOG 018. **Siguiente: veredicto del arquitecto sobre 9.2-C → congelar parte técnica de Fase 9 → material humano de Sebastián (P1 total del mapa 9.2-A)** | VERIFIED |
